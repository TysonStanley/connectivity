...
#' @param side Visualize the side angle of the brain? Default is TRUE (and currently the only option)
#'
#' @import ggplot2
#' @import dplyr
...
#'
#' @export
brain_viz <- function(obj, jitter_val = .04, side = TRUE, regs = NULL){
  ...
  if (isTRUE(side)){
    ...
    ggplot2::ggplot(fig_data, ggplot2::aes(x, y)) +
      ...
      ggplot2::coord_cartesian(xlim = c(0,10),
                               ylim = c(0,10)) +
      ggplot2::theme_void() +
      ggplot2::scale_color_viridis_d(begin = .2, end = .9) +
    ...
    ## Not yet implemented (probably a top view of the brain?)
    stop("Only `side = TRUE` implemented at this point.", call. = FALSE)
